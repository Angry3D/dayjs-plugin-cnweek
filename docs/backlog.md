...
| --------- | ------ | --------- | -------- | ------------------------------------ | ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| TODO-0012 | P2     | done      | chore    | 增加代码质量脚本和静态检查           | npm 开源包最佳实践复核                | 当前已有 `build`、`test`、`typecheck`、`verify` 脚本，但仍没有 `lint` 或格式检查脚本                                          | 已使用现有 Prettier + TypeScript 能力补充 `format:check` 和 `lint`，并纳入 CI，待用户确认后归档。 |
