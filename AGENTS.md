...
- 只有具体事项已经完成执行并通过必要验证后，才创建 `docs/archive/` 归档。
- 即使自动化验证通过，也应等待用户体验确认或明确要求后再归档；除非用户提前授权自动归档。
- 单纯讨论、记录建议、补充 README、创建或整理 backlog 待办，不应归档为完成阶段，除非用户明确要求归档这次文档整理工作。
- 如果某次工作只是为后续实现做准备，应保留在 `docs/backlog.md` 或 `docs/progress.md`，不要写入 archive。
- 当某个事项仍以 `todo`、`planned`、`doing` 或 `blocked` 状态存在于 `docs/backlog.md` 时，不得为该事项创建完成归档。
...
- 不要把缺少依赖目录误判为源码或构建配置错误。
- 修改依赖或锁文件前，先确认确实需要调整包管理器相关约定。
